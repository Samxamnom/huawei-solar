...
impl DataType {
    ...
    fn convert<I>(&self, data: I, scale: f32) -> f32
    where I: IntoIterator<Item=u16> {
        let mut iter = data.into_iter();
        let val = match self {
            DataType::I16 => i16::from_be_bytes(iter.next().unwrap().to_be_bytes()) as f32,
            DataType::I32 => {
                let bytes0: [u8; 2] = iter.next().unwrap().to_be_bytes();
                let bytes1: [u8; 2] = iter.next().unwrap().to_be_bytes();
                let bytes: [u8; 4] = [bytes0[0], bytes0[1], bytes1[0], bytes1[1]];
                i32::from_be_bytes(bytes) as f32
            }
            DataType::U16 => iter.next().unwrap() as f32,
            DataType::U32 => {
                let bytes0: [u8; 2] = iter.next().unwrap().to_be_bytes();
                let bytes1: [u8; 2] = iter.next().unwrap().to_be_bytes();
                let bytes: [u8; 4] = [bytes0[0], bytes0[1], bytes1[0], bytes1[1]];
    ...
    'out: loop {
        ...
        for table in tables {
            ...
            match db_client.execute(&format!("INSERT INTO {} ({}) VALUES ({})",
                ...
                table.values.iter().map(|r| r.data_type.convert((r.address..r.address+r.data_type.size()).map(|addr| *values.get(&addr).unwrap()), r.scale))
